...
<{!! $tag !!}
    ...
    $getExtraAttributeBag()
        ->class([
            'fi-wi-stats-overview-stat flex  items-center relative rounded-xl bg-white px-4 p-3 shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10',
        ])
    }}
>
@if ($icon = $getIcon())
    <x-filament::icon style="margin-right:10px" :icon="$icon" class="fi-wi-stats-overview-stat-icon   h-8 w-8 text-gray-400 dark:text-gray-500" />

@endif
    <div class="grid gap-y-[5px]">
        <div class="flex items-center gap-x-2">

            <span class="fi-wi-stats-overview-stat-label  font-medium text-gray-500 dark:text-gray-400"
                style="font-size:10px">
                {{ $getLabel() }}
            </span>
        </div>

        <div class="fi-wi-stats-overview-stat-value  font-semibold tracking-tight text-gray-950 dark:text-white"
